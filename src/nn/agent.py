...
class DQN_KerasAgent(Agent) :
  def __init__(self, model, training_batch_size, env_input_dims, env_output_dims, eps_decayrate = 0.99999, eps_start = 0.9, eps_end= 0.05, gamma_f = 0.99):
  ...
  def train(self):
    ...
    minibatch = random.sample(
      self.memory, min(len(self.memory), self.training_batch_size))
    for state, action, reward, next_state, done in minibatch:
      #if next_state[0] == 12 : print ("goal", "from :" , state, "to : ", next_state, done, reward)
      state = np.expand_dims(state, axis=0)
      next_state = np.expand_dims(next_state, axis=0)

      y_target = self.model.predict(state)
      y_target[0][action] = reward if done else reward + self.gamma_f * np.max(self.model.predict(next_state)[0])
